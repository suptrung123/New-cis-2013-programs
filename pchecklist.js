...
var passwordCheckOld = function () 
{
    ...
    stringPassword = $("password").value; //get user input password to test
    ...
    //declare array of bad passwords

    stringOutput = "Congratulation Password is Safe";
 
    if (stringPassword.length < 8) //check to see if password is less than 8 characters long and check the array (loop) to see if the user password is included in the bad password list 
    {
    ...
    else 
    {
        for ( var i=0; i< arrayBadPasswords.length; i++) 
        {
            ...
            {
                stringOutput = "Password Matches bad Password List. Please try again."; //if Yes, return error message and break out of loop
                break; //else output password is good
            } 
        }
    }
    
    $("output").value = stringOutput; //output the results
};
